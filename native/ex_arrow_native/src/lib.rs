...
static EX_ARROW_RECORD_BATCH_TYPE: OnceLock<SyncResourceType<ExArrowRecordBatch>> =
    OnceLock::new();
...
impl ResourceTypeProvider for ExArrowSchema {
    fn get_type() -> &'static rustler::resource::ResourceType<Self> {
        &EX_ARROW_SCHEMA_TYPE
            .get()
            .expect("ExArrowSchema resource not initialized (on_load not run?)")
            .0
    }
}
impl ResourceTypeProvider for ExArrowRecordBatch {
    fn get_type() -> &'static rustler::resource::ResourceType<Self> {
        &EX_ARROW_RECORD_BATCH_TYPE
            .get()
            .expect("ExArrowRecordBatch resource not initialized (on_load not run?)")
            .0
    }
}
impl ResourceTypeProvider for ExArrowIpcStream {
    fn get_type() -> &'static rustler::resource::ResourceType<Self> {
        &EX_ARROW_IPC_STREAM_TYPE
            .get()
            .expect("ExArrowIpcStream resource not initialized (on_load not run?)")
            .0
    }
}
impl ResourceTypeProvider for ExArrowIpcFile {
    fn get_type() -> &'static rustler::resource::ResourceType<Self> {
        &EX_ARROW_IPC_FILE_TYPE
            .get()
            .expect("ExArrowIpcFile resource not initialized (on_load not run?)")
            .0
    }
}
...
fn on_load(env: Env, _: rustler::Term) -> bool {
    ...
    let Some(t) =
        open_struct_resource_type::<ExArrowRecordBatch>(env, "ExArrowRecordBatch\0", flags)
    else {
        return false;
    };
    ...
    let Some(t) =
        open_struct_resource_type::<ExArrowIpcStream>(env, "ExArrowIpcStream\0", flags)
    else {
        return false;
    };
    ...
    let Some(t) =
        open_struct_resource_type::<ExArrowIpcFile>(env, "ExArrowIpcFile\0", flags)
    else {
        return false;
    };
...
rustler::init!(
    ...
    [
        ...
        compute::compute_sort,
        // Parquet
        parquet::parquet_reader_from_file,
        parquet::parquet_reader_from_binary,
        parquet::parquet_stream_schema,
        ...
        adbc::adbc_connection_get_table_types,
        adbc::adbc_connection_get_table_schema,
        adbc::adbc_connection_get_objects,
        adbc::adbc_statement_new,
        adbc::adbc_statement_set_sql,
        adbc::adbc_statement_bind,
